...
# Buses, loads, lines, actuators, etc. are all stored as dictionaries within this object
    ...
    def __init__(self,modelpath,loadfolder,loadpath,actpath,timesteps,timestepcur,subkVbase_phg,subkVAbase,refphasor,Psat_nodes,Qsat_nodes,PVforecast,act_init={}):
        # import data
        ...
        self.actdict = actbuilder(self.modeldata, self.busdict, loadpath, timesteps, timestepcur, self.act_init)
...
# As of 7/31/2018, this class only supports cap banks.
    def __init__(self,desig,timesteps):
        ...
        self.Qgen = cp.Variable((3,timesteps), name=self.name + '_sync_capQ')
...
def Vsrcbuilder(modeldata, busdict):
    ...
    for idx, row in Vsrcsheet.iterrows():
        if row['bus A']:
            ...
            indkey = row['bus C'][:len(row['bus C'])-2] # for consistency, remove space from this column in the impedance mode
        ...
        if row['bus C'] and isinstance(row['bus C'],str): # for consistency, remove space from this column in the impedance mode
            Vsrcdict[indkey].phases.append('c')
        ...
        angle_phA = row['Angle_a (Degree)'] # for consistency, remove space from this column in the impedance mode
...
# 2to3 long to int in (isinstance(row['P1(kW)'],long)
        if (row['P1 (kW)']) and (not np.isnan(row['P1 (kW)'])) and (isinstance(row['P1 (kW)'],int) or isinstance(row['P1 (kW)'],float)): 
            ...
            # 9/30/19 CHANGE TO EXPECT ALL WITH A SINGLE SPACE
        ...
        # append to busdict
        if shuntdict[indkey].id[0:3] == 'cap':
            busdict[indkey].cap.append(shuntdict[indkey])
        if shuntdict[indkey].id[0:8] == 'sync_cap':
            busdict[indkey].sync_cap.append(shuntdict[indkey])
...
def actbuilder(modeldata, busdict, actpath, timesteps, timestepcur, act_init):
    ...
    for key,ibus in busdict.items():
        for ph in ibus.phases:
            ...
            if act_init != {}:
                if key in act_init:
                    if ph in act_init[key]:
                        if not (key in actdict):
                            actdict[key] = actuator(key, timesteps)
                            actdict[key].node = busdict[key]
                            ibus.actuators.append(actdict[key])
                        actdict[key].phases.append(ph)
            if 'gen_kW_' + key + '_' + ph in list(actdf.columns.values):
                if not (key in actdict):
                    actdict[key] = actuator(key, timesteps)
                    actdict[key].node = busdict[key]
                    ibus.actuators.append(actdict[key])
    ...
    #set actuator capacities
    for key, iact in actdict.items():
        for ph in iact.phases:
            for ts in range(0,timesteps):
                if key not in act_init:
                    try:
                        actname_kVA = 'act_kVA_' + key + '_' + ph
                        actP = actdf[actname_kVA][ts]
                        actS = actP
                        actQ = 0 #not actually setting Q to 0, type 'act' doesnt use Qsched.
                        iact.type = 'act'
                    except:
                        actname_kW = 'gen_kW_' + key + '_' + ph
                        actP = actdf[actname_kW][ts]
                        actname_kVAR = 'gen_kVAR_' + key + '_' + ph
                        actQ = actdf[actname_kVAR][ts]
                        actS = actP + actQ
                        iact.type = 'gen'
                if key in act_init:
                    actP = act_init[key][ph]
                    actS = actP
                    actQ = 0 #not actually setting Q to 0, type 'act' doesnt use Qsched.
                    iact.type = 'act'

                if ph == 'a':
                    iact.Psched[0,ts] = actP
                    iact.Ssched[0,ts] = actS
                    iact.Qsched[:,ts] = actQ
                if ph == 'b':
                    iact.Psched[1,ts] = actP
                    iact.Ssched[1,ts] = actS
                    iact.Qsched[:,ts] = actQ
                if ph == 'c':
                    iact.Psched[2,ts] = actP
                    iact.Ssched[2,ts] = actS
                    iact.Qsched[:,ts] = actQ
...
def remove_shunt(feeder, bus):
    # removes shunt capacitors at "bus"
    indkey = str(bus)
    try:
        removed_shunt = feeder.shuntdict.pop(indkey)
        feeder.network = network_mapper(feeder.modeldata,feeder.busdict,feeder.linedict,feeder.transdict,feeder.switchdict)
    except KeyError:
        print("shunt", indkey, " is not in the feeder")

def remove_line(feeder, from_bus, to_bus):
    # removes lines for all phases between "from_bus" and "to_bus"
    from_bus = str(from_bus)
    to_bus = str(to_bus)
    indkey = from_bus + "to" + to_bus
    indkey_reversed = to_bus + "to" + from_bus
    try:
        removed_line = feeder.linedict.pop(indkey)
        feeder.busdict[from_bus].edges_out.remove(removed_line)
        feeder.busdict[to_bus].edges_in.append(removed_line)
        feeder.network = network_mapper(feeder.modeldata,feeder.busdict,feeder.linedict,feeder.transdict,feeder.switchdict)
        set_per_unit(feeder.linedict)
    except KeyError:
        print("line", indkey, " is not in the feeder")
        if indkey_reversed in feeder.linedict:
            print("found", indkey_reversed, "in the feeder. Try reversing from and to bus in call to remove function")
    

def remove_switch(feeder, from_bus, to_bus):
    # removes switches for all phases between "from_bus" and "to_bus"
    from_bus = str(from_bus)
    to_bus = str(to_bus)
    indkey = from_bus + "to" + to_bus
    indkey_reversed = to_bus + "to" + from_bus
    try:
        removed_switch = feeder.switchdict.pop(indkey)
        feeder.busdict[from_bus].edges_out.remove(removed_switch)
        feeder.busdict[to_bus].edges_in.append(removed_switch)
        feeder.network = network_mapper(feeder.modeldata,feeder.busdict,feeder.linedict,feeder.transdict,feeder.switchdict)
    except KeyError:
        print("switch", indkey, "is not in the feeder")
        if indkey_reversed in feeder.switchdict:
            print("found", indkey_reversed, "in the feeder. Try reversing from and to bus in call to remove function")
    

def remove_transformer(feeder, from_bus, to_bus):
    # removes transformers for all phases between "from_bus" and "to_bus"
    from_bus = str(from_bus)
    to_bus = str(to_bus)
    indkey = from_bus + "to" + to_bus
    indkey_reversed = to_bus + "to" + from_bus
    try:
        removed_transformer = feeder.transdict.pop(indkey)
        feeder.busdict[from_bus].edges_out.remove(removed_transformer)
        feeder.busdict[to_bus].edges_in.append(removed_transformer)
    except KeyError:
        print("transformer", indkey, "is not in the feeder")
        if indkey_reversed in feeder.transdict:
            print("found", indkey_reversed, "in the feeder. Try reversing from and to bus in call to remove function")
...
def remove_component(base_feeder, comp_id):
    ...
    elif comp_id.startswith("switch"):
        from_bus = comp_id[6:].split("to")[0]
        to_bus = comp_id[6:].split("to")[1]
        remove_switch(mod_feeder, from_bus, to_bus)
